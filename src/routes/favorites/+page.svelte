...
<div class="content">
  ...
  <p class="center-text-start">
    Hier kannst du deine Lieblings-Yu-Gi-Oh! Karten sammeln und verwalten. Füge
    Karten hinzu, schreibe Kommentare und organisiere deine Sammlung!
  </p>
  ...
  {#if data.favoriteCards && data.favoriteCards.length > 0}
    <h2 class="section-title">
      Deine Lieblingskarten ({data.favoriteCards.length})
    </h2>
      ...
      {#each data.favoriteCards as card}
        <div class="card-item">
          ...
          <div class="card-details">
            ...
            {#if card.addedDate}
              <p class="card-date">
                Hinzugefügt: {new Date(card.addedDate).toLocaleDateString(
                  "de-DE"
                )}
              </p>
            {/if}
              ...
              <form
                ...
                use:enhance={() => {
                  return ({ cancel }) => {
                    if (
                      !confirm(
                        "Möchtest du diese Karte wirklich aus deinen Favoriten entfernen?"
                      )
                    ) {
                      cancel();
                    }
  ...
  {:else}
    ...
    <div
      class="intro-section"
      style="text-align: center; max-width: 600px; margin: 40px auto;"
    >
      <h2 class="section-title">Keine Lieblingskarten vorhanden</h2>
      <p>Du hast noch keine Karten zu deinen Favoriten hinzugefügt.</p>
      <p>
        Klicke auf "Neue Karte hinzufügen", um deine erste Lieblingskarte zu
        speichern!
      </p>
    </div>
  {/if}
